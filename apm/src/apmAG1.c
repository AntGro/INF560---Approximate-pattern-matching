...
int main(int argc, char **argv) {
    ...
    if (rk == 0) {
        ...
        toSend = n_bytes / n;
        *bufRec = malloc(sizeof(char)*toSend);

        MPI_Scatter(buf, toSend, MPI_CHAR, bufRec, toSend, MPI_CHAR, 0, MPI_COMM_WORLD);
    }
    else {
        MPI_Bcast(&n_bytes, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    n_bytes = sizeof(bufRec)/sizeof(bufRec[0]);
    ...
    ******/
    ...
    if (rk == 0) {
        ...
        MPI_Reduce(n_matches_loc, n_matches, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
        /* Timer stop */
